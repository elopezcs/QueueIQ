...
def _build_specs() -> tuple[LaunchSpec, ...]:
    ...
    return (
        ...
        LaunchSpec(
            key="queuecontrol-simulator",
            name="QueueControl Simulator",
            cwd=ROOT_DIR / "QueueControl",
            command=(python, "queue-simulation/backend-queue-simulation.py"),
            startup_delay=2.0,
            helper=True,
        ),
        LaunchSpec(
            key="queuecontrol-backend",
            name="QueueControl Backend",
            cwd=ROOT_DIR,
            command=(
                python,
                "-m",
                ...
                "8001",
                "--app-dir",
                str(ROOT_DIR / "QueueControl" / "backend"),
            ),
        ...
        LaunchSpec(
            ...
            command=(
                ...
                "streamlit",
                "run",
                "queue-simulation/frontend-dashboard.py",
                "--server.address",
                LOCAL_HOST,
                "--server.port",
                "8501",
...
def run_workspace_launcher() -> int:
    ...
    print("This will launch Chatbot frontend/backend plus QueueControl simulator, backend, and dashboard.")
    print()
    ...
    try:
        ...
        print("- Chatbot backend:  http://127.0.0.1:8000/docs")
        print("- QueueControl API: http://127.0.0.1:8001/docs")
        print("- QueueControl UI:  http://127.0.0.1:8501")
        print()
...
def render_workspace_page() -> None:
    ...
    with setup_tab:
        ...
        st.write(
            "The root launcher starts Chatbot frontend/backend plus QueueControl simulator, backend, "
            "and dashboard using the same Python environment."
        )
