...
def has_chat_session_id(chat_session_id: object) -> bool:
	if pd.isna(chat_session_id):
		return False
...
def fetch_traceability_summary(chat_session_id: str) -> dict[str, object]:
	...
	try:
		...
		try:
			...
			error_payload = {"detail": response_body or f"HTTP {exc.code}"}
		detail = error_payload.get("detail", f"HTTP {exc.code}")
		raise RuntimeError(f"Traceability request failed: {detail}") from exc
	except urllib.error.URLError as exc:
		raise RuntimeError(
			f"Unable to reach the QueueIQ API at {QUEUECONTROL_API_BASE_URL}."
		) from exc

	if not isinstance(payload, dict):
		raise RuntimeError("Traceability endpoint returned an unexpected response format.")

	return payload


@st.dialog("Chat Detail")
def render_chat_detail_dialog(
	chat_session_id: str,
	*,
	payload: dict[str, object] | None = None,
	error_message: str | None = None,
) -> None:
	st.caption(f"chat_session_id: {chat_session_id}")
	if error_message:
	...
	if st.button("Close", key=f"close_chat_detail_{chat_session_id}"):
		st.rerun()
...
def render_reception_queue_actions(
	...
	for _, patient in df_waiting.iterrows():
		...
		if has_chat_session_id(chat_session_id):
