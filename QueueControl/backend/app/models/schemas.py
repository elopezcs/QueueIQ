...
class QueuePatientOut(BaseModel):
    ...
    acuity: int = Field(ge=1, le=5, description="Patient acuity, where 1 is most urgent and 5 is least urgent.")
...
class ClinicQueueSummaryOut(BaseModel):
    ...
    next_patient_acuity: int | None = Field(default=None, ge=1, le=5, description="Acuity of the next patient in queue.")
...
class PatientCreateIn(BaseModel):
    acuity: int = Field(default=3, ge=1, le=5, description="Acuity to assign to the new patient.", examples=[3])
...
class PatientCreateOut(BaseModel):
    ...
    acuity: int = Field(ge=1, le=5, description="Acuity assigned to the new patient.")
    est_duration: int = Field(ge=1, description="Estimated service duration in seconds.")
